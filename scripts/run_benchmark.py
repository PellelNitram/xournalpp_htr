...
if __name__ == "__main__":
    ...
    if args["format"] == "json":
        ...
        print(f"Search Q : {result.search_quality:.1%}  (recall × (1 − CER_ci))")
        print(
            f"Exact    : {result.exact_match_rate:.1%}  ({round(result.exact_match_rate * result.n_matched)}/{result.n_matched} matched words)"
        )
        if args["html_report"] is not None:
